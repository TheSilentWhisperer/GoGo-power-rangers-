...
func NewPuctAgent(simulations_per_move int, nb_routines int, max_parallel_searches int, resign_threshold float64, client remote_trainer.PositionEvaluatorClient) *MctsAgent {
	var agent *MctsAgent = &MctsAgent{
		...
		BackpropagationQueue: utils.NewLockedQueue[utils.Triple[MctsNode, float64, *environment.Game]](max_parallel_searches),
